<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            ...
            <String label="고객이름" v-model="value.customerName" :editMode="editMode" :inputUI="'TEXT'"/>
            <Number label="총금액" v-model="value.totalAmount" :editMode="editMode" :inputUI="'TEXT'"/>
        </v-card-text>
	...
	async created() {
        ...
        methods: {
            ...
            async updateOrder(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['/orders/{orderNumber}'].href), params)
