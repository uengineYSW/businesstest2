<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            ...
            <Number label="누적금액" v-model="value.amoutpayment" :editMode="editMode" :inputUI="''"/>
            <String label="멤버등급" v-model="value.membergrade" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
	...
	async created() {
        ...
        methods: {
            ...
            async updateUser(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['api/users/{id}'].href), params)
